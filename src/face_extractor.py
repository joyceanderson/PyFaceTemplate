...
class FaceTemplateExtractor:
    ...
    def __init__(self, detection_model='scrfd_10g_gnkps', recognition_model='glintr100',
        ...
        try:
            ...
            # Detection
            # Detection (FaceAnalysis) — prefer local if provided
            if models_dir and os.path.exists(models_dir):
                # Normalize root: if user passes ".../models", strip the trailing "models"
                root_path = models_dir
                if os.path.basename(models_dir) == "models":
                    root_path = os.path.dirname(models_dir)

                logging.info(f"Using models from local directory: {root_path}")
                self.app = FaceAnalysis(
                    name=detection_model,
                    root=root_path,                 # <-- always parent, InsightFace will append "models/"
                    providers=providers
                )
            else:
                # Fall back to default behavior (download from internet)
                logging.info(
                    f"No local models found. Using default detection model: {detection_model}")
                self.app = FaceAnalysis(
                    name=detection_model, providers=providers)
            ...
            if models_dir:
                candidate = os.path.join(
                    models_dir, f"{recognition_model}.onnx")
                if os.path.exists(candidate):
                    rec_model_path = candidate
                    logging.info(
                        f"Loading recognition model from: {rec_model_path}")

            # Special handling for buffalo_l model which is a built-in model
            if recognition_model == 'buffalo_l':
                logging.info("Using built-in buffalo_l model for recognition")
                self.model = None
            elif rec_model_path:
                ...
                logging.info(
                    f"Using default recognition model (by name): {recognition_model}")
    ...
    def extract(self, image_path, return_face_data=False):
        ...
        try:
            ...
            if self.model is None:
                ...
                # Build the same 112x112 aligned crop used by recognition
                if getattr(face, "kps", None) is not None:
                    aligned = norm_crop(
                        img, landmark=face.kps, image_size=112)  # BGR
                elif getattr(face, "landmark_2d_106", None) is not None:
                    aligned = norm_crop(
                        img, landmark=face.landmark_2d_106, image_size=112)  # BGR
                else:
                    x1, y1, x2, y2 = map(int, face.bbox)
                    aligned = cv2.resize(
                        img[y1:y2, x1:x2], (112, 112))  # BGR fallback

                face_img = cv2.cvtColor(aligned, cv2.COLOR_BGR2RGB)
            ...
            # Return the aligned 112x112 crop that was passed to the model
            if return_face_data:
...
def process_batch(batch_data):
    ...
    try:
        ...
        logging.error(
            f"Failed to initialize face extractor on GPU {gpu_id}: {str(e)}")
        return [], []
    ...
    for image_file in image_files:
        ...
        # Extract template
        if args.save_crops:
            template, face_crop = extractor.extract(
                image_file, return_face_data=True)
        else:
...
def process_images(args):
    ...
    missing_file = os.path.join(
        args.output, 'summary', 'missing_templates.txt')
    summary_directory = Path(args.output).joinpath("summary")
    crops_directory = Path(args.output).joinpath(
        "crops") if args.save_crops else None
    ...
    if args.use_gpu and args.num_gpus > 1:
        ...
        # Don't use more GPUs than images
        num_gpus = min(args.num_gpus, len(image_files))
        batch_size = len(image_files) // num_gpus
        batches = []

        for i in range(num_gpus):
            start_idx = i * batch_size
            end_idx = start_idx + batch_size if i < num_gpus - \
                1 else len(image_files)
            gpu_batch = (image_files[start_idx:end_idx],
                         args, i % args.num_gpus)
        ...
        logging.info(
            f"Using {'GPU ' + str(gpu_id) if args.use_gpu else 'CPU'} for processing")
        ...
        try:
            ...
            logging.error(
                "Please make sure you have the required models downloaded.")
            return
            ...
            for image_file in image_files:
                ...
                if template is not None:
                    ...
                    if not args.exclude:
                        ...
                        template_path = os.path.join(
                            args.output, 'templates', f"{filename}.npy")
                        np.save(template_path, np.zeros(
    ...
    processing_speed = len(image_files) / \
        processing_time if processing_time > 0 else 0
    logging.info(
        f"Took: {processing_time:.3f} s. ({processing_speed:.3f} im/sec)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--threads', default=4,
                        type=int, help='number of worker processes')
    parser.add_argument('-b', '--batch', default=1, type=int,
                        help='number of images per batch')
    parser.add_argument('-d', '--dir', type=str,
                        help='path to directory with images', default='./sample')
    parser.add_argument('-o', '--output', type=str,
                        help='directory where templates are saved', default="./output")
    parser.add_argument('--exclude', action='store_true',
                        help="exclude images with no face detected")
    parser.add_argument('--save-crops', action='store_true',
                        help="saves cropped faces when detected")
